...
`;
    ...
    function calcularCharola( ){
        ...
        let numCablesTotales = datosCables.length;
        let a= 0,b= 0,c= 0,d = 0;
        for(let i=0; i < datosCables.length; i++){
            let calibre = datosCables[i][3];
            if( calibre === `4/0` || calibre === `3/0` || calibre === `2/0` || calibre === `1/0` ) //Caso d
                d = d + 1;                
            else{
                calibre = parseInt(calibre);
                if( calibre >= 1000) //Caso A
                    a = a + 1;
                else
                    if( calibre >= 250 && calibre <= 900 ) //Caso B
                        b = b + 1;
                    else
                        if( calibre >= 1 && calibre <= 4 ) //Caso B
                            d = d + 1;                                
            }            
        }
        ...
        let resultado = "";
        //Acciones con los casos
        if( a > 0 && b == 0 && d == 0 ) // Caso A
            resultado = casoA( datosCables );
        else
            if( b > 0 && a == 0 && d == 0 ) // Caso B
                resultado = casoB( datosCables );
            else
                if( a > 0 && b > 0 && d == 0 ) // Caso C
                    resultado = casoC( datosCables );
                else
                    if( a == 0 && b == 0 && d > 0 ) // Caso D
                        resultado = casoD( datosCables );
        ...
        div.innerHTML = `
            <table class="tablaResultados" id="tablaResultados">
                ...
                <tr id="fila1"><td class="colum1"> Area de cables </td><td>` + resultado[0] + ` cm </td></tr>
                <tr id="fila1"><td class="colum1"> Tamaño charola </td><td>` + resultado[1]+`</td></tr>
            </table>`;  
    ...
    function casoA( datosCables ){// Caso A

        return false;
    }
    function casoB( datosCables ){// Caso B
        let sumaArea = 0;
        for(let i=0; i<datosCables.length; i++){
            sumaArea += buscaAreaCable( datosCables[i] );
        }
        sumaArea = sumaArea.toFixed(2);
        let resultado = buscaTamañoArea( sumaArea );
        return [ resultado, sumaArea ];
    }
    function casoC( datosCables ){ // Caso C
        return false;
    }
    function casoD( datosCables ){// Caso D
        return false;
    }

    function buscaTamañoArea( sumaArea ){
        let mejor = tablaCha[0][0];
        let j = 0;
        for( j = 0; j < tablaCha.length; j++){    
            let valorTabla = parseInt( tablaCha[j][1] );
            if(  sumaArea <= valorTabla ){
                mejor = tablaCha[j][0];
                break;
            }else{
                if( j == tablaCha.length - 1){
                    mejor = -1;
                }
            }
        }
    ...
    function dametablaCha( ){
        let token = "";
        let caracter = "";
        let tabla = [];
        let registro = [];
        for(let i=0; i<tablaChaCadena.length;i++){
            let caracterAscii = tablaChaCadena.charCodeAt(i);
            let caracter = tablaChaCadena[i];
            //alert("caracter= "+ caracter +" CaracterAscii= "+ caracterAscii);
            if( caracterAscii !== 44 && caracterAscii !== 10){
                //if( caracterAscii !== 32)//Para quitar espacios
                    token += caracter;
